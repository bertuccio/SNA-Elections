...
# Retweet Graph
sqlite3 -csv -separator '(a@a)' temp.db "select source, users.screen_name as target from users, tweets, (select users.screen_name as source, retweet_id as r from tweets, users where retweet_id<>-1 and users.id=user_id) where r=tweets.id and user_id=users.id;" > resources/retweet_out.csv
...
sqlite3 -csv temp.db "select source, users.name as target from users, (select users.name as source, quoted_user_id as q from tweets, users where quoted_user_id<>-1 and users.id=user_id) where q=users.id;" > resources/quote_out.csv
